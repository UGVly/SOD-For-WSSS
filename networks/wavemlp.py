...
model_urls = {
    'wave_S': 'https://hub.nuaa.cf/huawei-noah/CV-Backbones/releases/download/wavemlp/WaveMLP_S.pth.tar'
}
...
class WaveNet(nn.Module):
    ...
    def forward_tokens(self, x):
        ...
        # return outs
        # print('self.fork_feat',self.fork_feat)
        if self.fork_feat:
            return outs
        return x
...
def WaveMLP_T_dw(pretrained=False, **kwargs):
    ...
    model = WaveNet(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                     fork_feat=True,mlp_ratios=mlp_ratios,mode='depthwise', **kwargs)
    # print('model',model)
...
def WaveMLP_T(pretrained=False, **kwargs):
    ...
    model = WaveNet(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                    fork_feat=True, mlp_ratios=mlp_ratios, **kwargs)
    model.default_cfg = default_cfgs['wave_T']
    return model
...
def WaveMLP_S(pretrained=False, **kwargs):
    ...
    embed_dims = [64, 128, 320, 512]
    model = WaveNet(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                     fork_feat=True, mlp_ratios=mlp_ratios,norm_layer=MyNorm, **kwargs)
    model.default_cfg = default_cfgs['wave_S']
    if pretrained:
        url = model_urls['wave_S']
...
def WaveMLP_M(pretrained=False, **kwargs):
    ...
    embed_dims = [64, 128, 320, 512]
    model = WaveNet(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                     fork_feat=True, mlp_ratios=mlp_ratios,norm_layer=MyNorm,ds_use_norm=False, **kwargs)
