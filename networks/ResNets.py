...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000,fork_feat=True):  # layers=参数列表 block选择不同的类
        self.inplanes = 64 
...
def build_resnet_model(config):
    ...
    model = ResNet(Bottleneck, config.RESNET.LAYERS)
    if pretrained:
        ckpt_path = cfg_dict[config.MODEL.NICKNAME][0]
        ckpt = torch.load(ckpt_path)
        model.load_state_dict(ckpt,strict=False)

    return model,config.RESNET.EMBED_DIMS
