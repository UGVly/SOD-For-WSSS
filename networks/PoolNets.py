...
model_urls = {
    "poolformer_s12": "https://hub.nuaa.cf/sail-sg/poolformer/releases/download/v1.0/poolformer_s12.pth.tar",
    "poolformer_s24": "https://hub.nuaa.cf/sail-sg/poolformer/releases/download/v1.0/poolformer_s24.pth.tar",
    "poolformer_s36": "https://hub.nuaa.cf/sail-sg/poolformer/releases/download/v1.0/poolformer_s36.pth.tar",
    "poolformer_m36": "https://hub.nuaa.cf/sail-sg/poolformer/releases/download/v1.0/poolformer_m36.pth.tar",
    "poolformer_m48": "https://hub.nuaa.cf/sail-sg/poolformer/releases/download/v1.0/poolformer_m48.pth.tar",
}
...
def poolformer_s24(pretrained=False, **kwargs):
    ...
    model = PoolFormer(
        layers, embed_dims=embed_dims, 
        mlp_ratios=mlp_ratios, downsamples=downsamples,fork_feat=True,
        **kwargs)
    model.default_cfg = default_cfgs['poolformer_s']
    if pretrained:
...
def build_pool_model(config):
    model_type = config.MODEL.TYPE
    
    pretrained = config.MODEL.PRETRAINED

    if model_type == 'pool':
        model = PoolFormer(config.POOL.LAYERS, embed_dims=config.POOL.EMBED_DIMS, 
                mlp_ratios=config.POOL.MLP_RATIOS, downsamples=config.POOL.DOWNSAMPLES, 
                layer_scale_init_value=1e-6, 
                fork_feat=True)
    ...
    return model,config.POOL.EMBED_DIMS