...
class NAMSwinNet(nn.Module):
    def __init__(self,config):
        super(NAMSwinNet, self).__init__()

        self.depth_scale = nn.Conv2d(1,3,2,2)

        self.encoderR, embed_dims = build_backbone(config)#swin_like_model

        self.encoderD, _ = build_backbone(config)
    ...
    def forward(self, rgb,x):
        ...
        y0,y1,y2,y3 = self.encoderD(self.depth_scale(x))
        ...
        return s1,s2,s3,s4,edge_sod,edge_rgb,lat_loss
...
if __name__ == "__main__":
    ...
    model = NAMSwinNet(config).cuda()

    rgb = torch.randn(4,3,224,224).cuda()
    x = torch.randn(4,1,448,448).cuda()
